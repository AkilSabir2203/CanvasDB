...
import { Attribute } from '@/app/libs/types';
    ...
    return (
        <>
            ...
            <div className='border border-foreground rounded p-1 bg-background'>
                <Input value={data.name} placeholder='Model Name' name="name" variant='underlined' onChange={onChange} className='bg-purple-100 dark:bg-purple-950 rounded-sm pl-1' />
                {data.open && <div className='mt-2 flex flex-col gap-2'>
                    {data.attributes.map((attr, index) => (
                        <div key={index} className='grid grid-cols-[2fr_1fr_0.25fr] gap-2'>
                            <Input name="name" placeholder='Field Name' value={attr.name} onChange={(e) => onAttributeChange(index, e.target.name, e.target.value)} />
                            ...
                            <Button variant="destructive" onClick={() => onRemoveAttribute(index)} className="bg-purple-600">
                                <Trash2Icon size={20} />
                            </Button>
                        </div>
                    ))}
                ...
                <div className='flex gap-2 mt-2'>
                    ...
                    <Button variant="destructive" onClick={deleteNode} className='w-full mt-2 bg-purple-600'>
                        <Trash2Icon />
                    </Button>
                </div>
            </div>
